...
func (h *Handler) GetPriorityConfiguration(w http.ResponseWriter, r *http.Request) {
	var applicationErr error
	var resp *dto.PriorityConfig
	resp, applicationErr = h.svc.GetPriorityConfiguration(
		r.Context(),
	)
	if applicationErr != nil {
		h.responseErrorHandler.HandleAndWriteError(w, r, applicationErr)
...
func (h *Handler) UpdatePriorityConfiguration(w http.ResponseWriter, r *http.Request) {
	var applicationErr error
	req := dto.PriorityConfig{}
	applicationErr = json.NewDecoder(r.Body).Decode(&req)
	if applicationErr != nil {
		h.responseErrorHandler.HandleAndWriteError(w, r, ngErrors.New(ngErrors.InternalServerErrorCode, "problem decoding request body", http.StatusInternalServerError, nil))
		return
	...
	var resp *dto.PriorityConfig
	resp, applicationErr = h.svc.UpdatePriorityConfiguration(
		r.Context(), &req,
	)
	if applicationErr != nil {
		h.responseErrorHandler.HandleAndWriteError(w, r, applicationErr)
...
func (h *Handler) GetDefaultPriorityConfiguration(w http.ResponseWriter, r *http.Request) {
	var applicationErr error
	var resp *dto.PriorityConfig
	resp, applicationErr = h.svc.GetDefaultPriorityConfiguration(
		r.Context(),
	)
	if applicationErr != nil {
		h.responseErrorHandler.HandleAndWriteError(w, r, applicationErr)
